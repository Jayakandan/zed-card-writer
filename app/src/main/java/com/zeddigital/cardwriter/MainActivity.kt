...
class MainActivity : Activity() {
    ...
    /** "Top up amount" - writes the name (if entered) and the balance. */
    private fun doSetBalance(mifare: MifareClassic) {
        ...
        if (name.isNotEmpty()) {
            if (!writeAndVerify(mifare, CardFormat.NAME_BLOCK, CardFormat.encodeName(name), "name")) {
     ...
     *  writes the name too if one was entered, then reports the top-up to the database. */
    private fun doTopUp(mifare: MifareClassic) {
        ...
        if (newName.isNotEmpty()) {
            writeAndVerify(mifare, CardFormat.NAME_BLOCK, CardFormat.encodeName(newName), "name")
        }
